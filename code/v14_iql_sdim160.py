...
from make_training_dataset_pickle_policy import SlidingWindowQueue
...
class TrainConfig:
    ...
    project: str = "BWE"
    group: str = "IQL"
    name: str = f"new_act-K1000-beta-{beta}-sdim151-short"
...
class ReplayBuffer:
    def __init__(
        ...
        self._states = torch.zeros(
            (buffer_size, 160), dtype=torch.float32, device="cpu"
        )
    ...
    def load_dataset(self, data: Dict[str, np.ndarray], sample_num: int = 2000000):
        ...
        #     )
        # sample_num = 2000000
        new_act = torch.clamp(self._to_tensor(data["actions"][:sample_num] / 1e6), min=0.01, max=8.0)
    ...
    def sample(self, batch_size: int) -> TensorBatch:
        ...
        # 拼接
        states = torch.cat([states[:, :1], states[:, 10:]], dim=1) 
        next_states = torch.cat([next_states[:, :1], next_states[:, 10:]], dim=1)
        # 将states增加一个维度
...
###       
        # mean = mean.clamp(min = 10)  # larger than 10bps
        # mean = torch.exp(mean) * last_act
        mean = torch.exp(((mean + 1.0) / 2 * ln_800 + ln_001)) * 1e6  # Mbps -> bps
        mean = mean.clamp(min = 10)  # larger than 10bps
        std = torch.exp(self.log_std.clamp(LOG_STD_MIN, LOG_STD_MAX))
        std = std.expand(mean.shape[0], 1)
        ret = torch.cat((mean, std), 1)
...
def get_input_from_file():
    ...
    action_queue = SlidingWindowQueue()
    new_obs = []
    for idx in range(len(call_data['observations']) - 1):
        idx += 1
        action_queue.add_value(call_data['bandwidth_predictions'][idx - 1])
        last_action_short = action_queue.get_recent_1()
        # last_action_long = action_queue.get_recent_5_averages()
        new_obs.append(last_action_short + call_data['observations'][idx])

    # observations = np.asarray(call_data['observations'], dtype=np.float32)
    observations = np.asarray(new_obs, dtype=np.float32)
...
def export2onnx(pt_path, onnx_path):
    ...
    with torch.no_grad():
        for i in tqdm(range(dummy_inputs.shape[1]), desc="Verifing  "):
            ...
            assert np.allclose(torch_estimate.numpy(), onnx_estimate, atol=1), 'Failed to match model outputs!, {}, {}'.format(torch_estimate.numpy(), onnx_estimate)
...
def evaluate(onnx_path):
    ...
    for f_path in tqdm(small_evaluation_datasets, desc="Evaluating"):
        ...
        # observations = np.asarray(call_data['observations'], dtype=np.float32)
        observations = np.asarray(new_obs, dtype=np.float32)
        true_capacity = np.asarray(call_data['true_capacity'][1:], dtype=np.float32)

        model_predictions = []
        hidden_state, cell_state = np.zeros((1, 1), dtype=np.float32), np.zeros((1, 1), dtype=np.float32)
        # pre_act = observations[0][0]
        for t in range(observations.shape[0]):
            ...
            # if t == 0 :
            #     model_predictions.append(pre_act)
            # else:
...
def train(config: TrainConfig):
    ...
    for i in range(6):
        ...
        dataset = pickle.load(open(f"/data2/kj/Schaferct/training_dataset_pickle/training_dataset_K_1000_v{i}_sdim160.pickle", 'rb'))
        # if i == 5:
        #     replay_buffer.load_dataset(dataset, sample_num=int(dataset["observations"].shape[0] / 10))
        # elif i == 2:
